package course.catalog.access;

import java.util.Scanner;

import java.io.*;
import org.jsoup.Connection.Method;
import org.jsoup.Connection.Response;

import org.jsoup.Jsoup;
import org.jsoup.nodes.*;
import org.jsoup.examples.HtmlToPlainText;
import org.jsoup.select.Elements;



public class CourseCatalogAccess {
    
public static boolean isUpperCase(String s)
{
    for (int i=0; i<s.length(); i++)
    {
        if (!Character.isUpperCase(s.charAt(i)))
        {
            return false;
        }
    }
    return true;
}

    
    public static void main(String[] args) throws Exception {
           Document newDoc = Jsoup.connect("https://my.sa.ucsb.edu/Public/curriculum/coursesearch.aspx").get();
           PrintWriter writer = new PrintWriter("Majors.data", "UTF-8");
            
           //System.out.println(newDoc.select("center tr"));
           
           HtmlToPlainText test = new HtmlToPlainText();
           Scanner parsing = new Scanner(test.getPlainText(newDoc));
           
           
           while(parsing.hasNext()){
               String read = parsing.next();
               if(read.equals("FALL")) break;
               if(isUpperCase(read) || (read.contains("&") && read.length() != 1)){
                   writer.print(read + " "); 
                   String nexRead = parsing.next();
                   if(isUpperCase(nexRead))
                       writer.println(nexRead);
                   else
                       writer.print("\n");
                   
                
               }
           }
           writer.close();
           File openMe = new File("Majors.data");
           Scanner newparsing = new Scanner(openMe);
           
           Document recon = Jsoup.connect("https://my.sa.ucsb.edu/Public/curriculum/coursesearch.aspx").get();
           Document retcon = Jsoup.connect("https://my.sa.ucsb.edu/Public/curriculum/coursesearch.aspx").get();
           Element sA = retcon.select("input#__VIEWSTATE").first();
           //Elements sB = retcon.select("__VIEWSTATEGENERATOR");
           //Elements sC = retcon.select("__EVENTVALIDATION");
           
           
               System.out.println(sA.text());
  
           
           
           while(newparsing.hasNext()){
           recon = Jsoup.connect("https://my.sa.ucsb.edu/Public/curriculum/coursesearch.aspx")
                   .data("__VIEWSTATE", "/wEPDwUKMTE5NDUyMzA1NGQYAQUeX19Db250cm9sc1JlcXVpcmVQb3N0QmFja0tleV9fFgEFHmN0bDAwJHBhZ2VDb250ZW50JHNlYXJjaEJ1dHRvbjCkD99UtL6+UGjtqg6z1XD62J+cJHh2r6RBL4f9vmwG")
                   .data("__VIEWSTATEGENERATOR", "0D1CB3EA")
                   .data("__EVENTVALIDATION", "")
                   .data("ctl00_pageContent_courseList",newparsing.nextLine())
                   .data("ctl00_pageContent_quarterList", "2172")
                   .data("ctl00_pageContent_dropDownCourseLevels","Undergraduate")
                   .userAgent("Mozilla")
                   .method(Method.POST)
                   .execute()
                   .parse();
           Elements checker = recon.select("center");
           
            for(Element e : checker){
                System.out.println(e.text());
            }
           }

    }
}
    

